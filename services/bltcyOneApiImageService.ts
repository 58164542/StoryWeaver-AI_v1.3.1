/**
 * 柏拉图 One-API 图像生成（banana2 / 2K）
 * 端点: https://one-api.bltcy.top/v1/images/generations
 * 模型: gemini-3.1-flash-image-preview-2k
 *
 * 说明：
...
const BLTCY_ONE_API_URL = "https://one-api.bltcy.top/v1/images/generations";
const BLTCY_BANANA2_MODEL = "gemini-3.1-flash-image-preview-2k";
...
interface OneApiImagesResponse {
 ...
 */
export const generateImageWithBltcyBanana2 = async (
  prompt: string,
  aspectRatio: string = "16:9",
  referenceImages: ReferenceImage[] = [],
  projectId: string,
  onProgress?: ProgressCallback,
  model: string = BLTCY_BANANA2_MODEL
): Promise<string> => {
  ...
  for (const ref of referenceImages) {
    try {
      ...
      Logger.logInfo("BltcyOneApiImage 参考图上传成功", { name: ref.name, uploaded });
    } catch (e) {
      Logger.logError("BltcyOneApiImage", `参考图上传失败，已跳过: ${ref?.name ?? "unknown"}`, e);
    }
  }
  ...
  const requestBody: any = {
    ...
    prompt: fullPrompt,
    size: "2k",
    // OpenAI images API 常见字段：n / response_format
    n: 1,
    response_format: "url",
  };
  ...
  const response = await fetch(BLTCY_ONE_API_URL, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(`柏拉图 One-API 图片生成请求失败 (${response.status}): ${errorText}`);
  }

  const result = (await response.json()) as OneApiImagesResponse;
  ...
  throw new Error("未从柏拉图 One-API 响应中获取到图片（缺少 url/b64_json）");
};
